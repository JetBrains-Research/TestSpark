package org.jetbrains.research.testspark.langwrappers.test.manager
...
interface TestGeneratorProvider {
    fun getTestGenerator(): TestGenerator

    companion object {
        private val EP = LanguageExtension<TestGeneratorProvider>("org.jetbrains.research.testgenie.testGeneratorProvider")

        fun getTestGenerator(language: Language): TestGenerator? {
            println("TestGeneratorProvider getTestGenerator: $language")
            return EP.forLanguage(language)?.getTestGenerator()
        }
    }
}
