...
class JavaTopButtonsPanelFactory(private val project: Project) {
    ...
    private val testCasePanelFactories = arrayListOf<JavaTestCasePanelFactory>()
     ...
     */
    fun setTestCasePanelFactoriesArray(testCasePanelFactories: ArrayList<JavaTestCasePanelFactory>) {
    ...
    private fun toggleAllCheckboxes(selected: Boolean) {
        ...
        project.service<JavaTestCaseDisplayService>()
            .setTestsSelected(if (selected) project.service<JavaTestCaseDisplayService>().getTestCasePanels().size else 0)
    }
