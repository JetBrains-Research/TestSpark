...
object ToolUtils {
     ...
     */
    fun osJoin(vararg strings: String): String = strings.joinToString(sep.toString())
    ...
    fun getJavaVersion(path: String): Int? =
        """(1.)?(\d+)""".toRegex().find(CommandLineRunner.run(arrayListOf(path, "-version")).executionMessage)?.let {
    ...
    ): Boolean = errorMonitor.hasErrorOccurred() || isProcessCanceled(errorMonitor, indicator)

    fun isProcessCanceled(
        errorMonitor: ErrorMonitor,
        indicator: CustomProgressIndicator,
    ): Boolean {
