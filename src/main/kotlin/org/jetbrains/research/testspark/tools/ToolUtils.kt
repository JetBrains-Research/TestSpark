...
object ToolUtils {
    ...
    fun saveData(
        ...
        val testGenerator = TestGeneratorProvider.getTestGenerator(language.asIntellijLanguage())!!
        for (testCase in report.testCaseList.values) {
            val code = testCase.testCode
            testCase.testCode = testGenerator.generateCode(
                project,
    ...
    fun getJavaVersion(path: String): Int? {
        return """(1.)?(\d+)""".toRegex().find(CommandLineRunner.run(arrayListOf(path, "-version")).executionMessage)
            ?.let {
