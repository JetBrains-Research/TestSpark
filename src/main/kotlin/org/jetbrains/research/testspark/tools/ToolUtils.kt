...
import org.jetbrains.research.testspark.core.exception.CouldNotRetrieveJavaVersionException
...
object ToolUtils {
    ...
    fun getJavaVersion(path: String): Int {
        val x = """(1.)?(\d+)""".toRegex().find(CommandLineRunner.run(arrayListOf(path, "-version")).executionMessage)?.let {
            it.groupValues[2].toInt()
        }
        if (x == null) throw CouldNotRetrieveJavaVersionException()
        return x
    }
