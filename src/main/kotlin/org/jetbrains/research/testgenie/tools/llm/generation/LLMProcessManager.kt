    ...
    fun runLLMTestGenerator(
        ...
        while (!generatedTestsArePassing){
            ...
            if (requestsCount >= MAX_REQUESTS){
                llmErrorManager.display(TestGenieBundle.message("invalidGrazieResult"), project)
                break
            }
            ...
            if(!File(generatedTestPath).exists()){
                llmErrorManager.display(TestGenieBundle.message("savingTestFileIssue"), project)
            }
            ...
            if (!compilationResult.first) {
                llmErrorManager.displayWarning(TestGenieBundle.message("compilationError"), project)
                requestsCount ++
