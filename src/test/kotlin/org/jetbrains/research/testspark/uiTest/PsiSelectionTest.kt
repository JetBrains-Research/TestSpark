...
class PsiSelectionTest {
    ...
    /**
     * Makes the robot click on TestSpark action group.
     */
    private fun clickOnTestSparkActionGroup() {
        with(remoteRobot) {
            find<ComponentFixture>(byXpath("//div[@accessiblename='TestSpark' and @class='ActionMenu' and @text='TestSpark']")).click()
        }
    }
     ...
     */
    private fun assertThatTestSparkActionGroupIsNotVisible() {
        assertThatThrownBy { clickOnTestSparkActionGroup() }.isInstanceOf(WaitForConditionTimeoutException::class.java)
    }
        ...
        with(remoteRobot) {
            assertThat(
                find<ComponentFixture>(
                    byXpath("//div[@text='TestSpark']//div[contains(@text, '$text')]"),
