...
private const val TEST_FILE_NAME = "ArrayUtilsTest"

private const val PROJECT_NAME = "untitled"

private const val EMPTY_CLASS = """public class ArrayUtilsTest {
}
"""
...
class GeneratedTestsToolWindowTest {
    ...
    @Video
    @Test
    fun testGeneratedTestTab(remoteRobot: RemoteRobot): Unit =
        with(remoteRobot) {
    ...
    @Video
    @Test
    fun testApplyTestsButtonDeselectAll(remoteRobot: RemoteRobot): Unit =
        with(remoteRobot) {
            val ideaFrame = find(IdeaFrame::class.java, timeout = Duration.ofSeconds(15))
            ideaFrame.apply {
                // Deselect all and apply to testSuite.
                deselectAllApplyTestsToTestSuite(TEST_FILE_NAME)
                // Open the correct file.
                openProjectFile(TEST_FILE_NAME, PROJECT_NAME)
                ...
                Assertions.assertThat(editor.text).isEqualTo(EMPTY_CLASS)
    ...
    @Video
    @Test
    fun testApplyTestsButtonSelectAll(remoteRobot: RemoteRobot): Unit =
        with(remoteRobot) {
            val ideaFrame = find(IdeaFrame::class.java, timeout = Duration.ofSeconds(15))
            ideaFrame.apply {
                ...
                assertThatThrownBy {
                    ...
                    .hasMessageContaining(
                        "Exceeded timeout (PT2S) for condition function (Found more than one 'EditorFixture' by '//div[@accessiblename='Remove' and @class='JButton' and @text='Remove']'",
                    )

                // Select all and apply to testSuite.
                selectAllApplyTestsToTestSuite(TEST_FILE_NAME)
                ...
                assertThatThrownBy {
                    ...
                    .hasMessageContaining(
                        "Exceeded timeout (PT2S) for condition function (Failed to find 'EditorFixture' by '//div[@accessiblename='Remove' and @class='JButton' and @text='Remove']'",
                    )
                ...
                Assertions.assertThat(editor.text.length).isGreaterThan(EMPTY_CLASS.length)
    ...
    fun closeAll(remoteRobot: RemoteRobot): Unit =
        with(remoteRobot) {
            find(IdeaFrame::class.java, timeout = Duration.ofSeconds(60)).apply {
                deleteProject(TEST_FILE_NAME, PROJECT_NAME)
                clickOnToolWindow()
                closeProjectFile()
                closeProject()
            }
